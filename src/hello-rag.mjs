...
for (const question of questions) {
  ...
  retirevedDocs.forEach((doc, index) => {
    const score = scoredResults[index] ? scoredResults[index][1].toFixed(4) : "N/A";
    console.log(`\n文档 ${index + 1} (相似度评分: ${score}):\n${doc.pageContent}`);
  });

  // 将检索到的文档内容拼接起来，作为模型的上下文
  const context = retirevedDocs.map(doc => doc.pageContent).join("\n\n");
}